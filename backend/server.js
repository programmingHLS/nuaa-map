...
function buildSystemPrompt() {
    return `你是南京航空航天大学校园地图的智能问答助手。
你的任务是基于提供的知识库（QA 问答、建筑信息）回答用户关于南航天目湖校区的问题。
请严格遵循以下规则：
1. 优先使用提供的知识库内容回答，不要编造信息。
2. 如果知识库中没有相关信息，坦诚告知用户"该问题暂未收录，建议咨询学校相关部门"。
3. 回答要简洁、准确，符合学生的语境和需求。
4. 回答中可以适当引导，比如"你可以在师生服务大厅X号窗口办理"。
5. 涉及开放时间、地点、办理流程等信息时，直接给出明确答案。
6. 使用中文回答。`;
}
...
function buildUserPrompt(question, qaResults, buildingResults, contextText) {
    ...
    if (buildingResults.length > 0) {
        const bText = buildingResults.map(r => {
            ...
            if (b.description) parts.push(`描述：${b.description}`);
...
async function callLLM(systemPrompt, userPrompt) {
    if (!LLM_API_KEY) {
        return {
            content: '服务端未配置 LLM_API_KEY，请联系管理员配置。',
